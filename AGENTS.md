...
| Probe pushed before a survey that reads the same URL | the dedup kept the probe, so nominatim/nws/census/nhl facts were all filed under `source-health`; travel-korea and sf-local sat at zero claims for 22 cycles while being read every cycle | surveys are planned first (`msl/tasks.py`); the probe stage skips an already-planned URL and hands its Accept header to the winning task |
| `max(persisted credit, row-provable count)` published per topic | the Library total matched no auditable quantity — neither the credit accumulator nor the ledger | two labelled fields: `verifiedClaims` (row-provable, recomputes from `claims.jsonl`) and `creditedClaims` (pre-schema-v2 history); see `msl/sitegen.py` |
| A register finding computed in one place and "reproduced" by a one-liner elsewhere | IRR-042 named topics with 62 claim credits as having "zero verified claims"; its own repro returned a different population | both call `pipeline.unsupported_topic_report`; the repro is `python3 -m msl.cli unsupported-topics` |
| Corroboration bucketed by `c.topic` alone | only family slugs matched, every family has 1 signal, and L1 published "a ratio of 1.00×" as a finding | `msl/learn.py` buckets by topic+subjects, excludes `sourceId == "derived"`, and reports claim rows *and* signals |
| A frozen date literal in a registry `probe_url` | the MLB probe re-read `date=2026-09-20` forever; the USGS window grew while its field name stayed | probe URLs are templates (`{today}`, `{owner_today}`, `{days_ago:N}`) rendered by `sources.render_probe_url` at read time; `tests/test_pipeline.py` sweeps every planned URL for unexplained dates |
