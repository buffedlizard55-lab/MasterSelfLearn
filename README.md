...
|---|---|
| Cycle | **3** |
| Verified claims in the ledger | **1,573** |
| — captured from live payloads | 1,448 |
| — derived by recorded arithmetic | 125 |
| Claims rejected by the evidence gate | **0** |
| Derived claims rechecked this cycle | 1 |
| Derived claims that no longer recompute | **0** |
| Topics in the library | **31** (new this cycle: 6) |
| Published insights | 125 |
| Forecasts issued this cycle | 355 |
| Forecasts scored against outcomes | 357 |
| Ideas in the competition | **4** (promoted: 0) |
| Open irregularities | **38** (new: 1) |
| Sources registered | 28 |
| — verified by a recorded live read | 8 |
| — currently blocked | 0 |
| Reads this cycle (ok / failed) | 11 / 0 |
| Bytes read this cycle | 0 |
| Manual inputs required | **0** |

Generated `2026-09-22T01:34:57Z` by `msl/pipeline.py`. Quoting any figure outside this block
...
|---|---|---|---|---|---|
| 1 | `S10_Persistence` | Persistence (null model) | 257 | 99.6% | +0.0 pts |
| 2 | `S02_MeanRevert` | Mean reversion | 34 | 0.0% | -99.6 pts |
| 3 | `S01_MomentumPersist` | Momentum persistence | 33 | 0.0% | -99.6 pts |
| 4 | `S05_EvidenceDensity` | Evidence density | 33 | 0.0% | -99.6 pts |

### Unranked

| Persona | Name | Why it is not ranked |
|---|---|---|
| `S03_Acceleration` | Acceleration | 1 scored forecast(s); 3 required before a rank means anything. |
| `S04_ConsensusFade` | Consensus fade | 1 scored forecast(s); 3 required before a rank means anything. |
...
|---|---|---|---|---|
| 1 | Replicate what made zai-org/ZCode grow | 0.400 | velocity-outlier | 6 |
| 2 | Replicate what made browser-use/jev-ultrafast grow | 0.400 | velocity-outlier | 6 |
| 3 | Replicate what made NandhaKishorM/laya grow | 0.400 | velocity-outlier | 6 |
| 4 | Watch for research lagging rulemaking on “artificial intelligence” | 0.275 | regulatory-lead | 1 |
...
| `L2` | The evidence gate has rejected 0 of 1573 attempted claims (0.00%). |
| `L3` | 5 of 31 tracked topics (16.1%) have at least one verified claim. |
| `L4` | Of 2 idea kinds, “velocity-outlier” holds the highest mean robustness (0.400 over 3 ideas). |
| `L5` | 0 of 8 registered sources are reading reliably (EMA ≥ 0.8); 0 are effectively unreadable (EMA < 0.2). |
...
28 registered, 8 verified by a recorded
live read, 0 blocked. 6 excluded
for requiring an API key, each with the reason recorded in `msl/sources.py`.
3 interest categories have no registered
