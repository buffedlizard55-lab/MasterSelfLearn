...
|---|---|
| Cycle | **1** |
| Verified claims in the ledger | **0** |
| — captured from live payloads | 0 |
| — derived by recorded arithmetic | 0 |
| Claims rejected by the evidence gate | **0** |
| Derived claims rechecked this cycle | 0 |
| Derived claims that no longer recompute | **0** |
| Topics in the library | **13** (new this cycle: 13) |
| Published insights | 0 |
| Forecasts issued this cycle | 0 |
| Forecasts scored against outcomes | 0 |
| Ideas in the competition | **0** (promoted: 0) |
| Open irregularities | **42** (new: 42) |
| Sources registered | 28 |
| — verified by a recorded live read | 8 |
| — currently blocked | 0 |
| Reads this cycle (ok / failed) | 0 / 0 |
| Bytes read this cycle | 0 |
| Manual inputs required | **0** |

Generated `2026-09-21T12:00:00Z` by `msl/pipeline.py`. Quoting any figure outside this block
...
|---|---|---|
| `S01_MomentumPersist` | Momentum persistence | 0 scored forecast(s); 3 required before a rank means anything. |
| `S02_MeanRevert` | Mean reversion | 0 scored forecast(s); 3 required before a rank means anything. |
| `S03_Acceleration` | Acceleration | 0 scored forecast(s); 3 required before a rank means anything. |
| `S04_ConsensusFade` | Consensus fade | 0 scored forecast(s); 3 required before a rank means anything. |
| `S05_EvidenceDensity` | Evidence density | 0 scored forecast(s); 3 required before a rank means anything. |
| `S06_MemoryWeighted` | Skill-weighted memory | 0 scored forecast(s); 3 required before a rank means anything. |
| `S10_Persistence` | Persistence (null model) | 0 scored forecast(s); 3 required before a rank means anything. |
...
5 warn, 36 info;
42 open, 0 resolved,
13 standing (structural limits that do not auto-resolve).
